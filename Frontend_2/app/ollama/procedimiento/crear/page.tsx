...
export default function CrearProcedimientoPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await procedimientoService.crearProcedimiento(payload)
  try { localStorage.removeItem(`draft:${pathname}`) } catch (e) {}
  setResult(res)
      toast({ title: 'Procedimiento creado', description: `ID: ${res.id}` })
    } catch (err: any) {
      toast({ title: 'Error', description: err?.message || 'Error al crear procedimiento' })
    } finally {
      setLoading(false)
    }
  }
  ...
  useEffect(() => {
    ...
    const state = { empresaId, usuarioId, protocoloId, pregunta }
    const serialized = JSON.stringify(state)
    const timer = window.setTimeout(() => {
      try {
        const any = [empresaId, usuarioId, protocoloId, pregunta].some((v) => isFilled(v))
        if (any) localStorage.setItem(key, serialized)
        else localStorage.removeItem(key)
      } catch (e) {}
    }, 400)
    return () => window.clearTimeout(timer)
  }, [empresaId, usuarioId, protocoloId, pregunta, pathname])

  return (
    <div className="max-w-2xl mx-auto py-12">
      <Card className="p-6">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-lg font-medium">Crear Procedimiento</h2>
          <DraftBackButton draftKey={`draft:${pathname}`} />
        </div>
        <FormDirtyAlert dirty={isFilled(empresaId) || isFilled(usuarioId) || isFilled(protocoloId) || isFilled(pregunta)} />
